...
{
    internal class ThinKlineParams : EmaDealParams
    {
        public double KlinesRatio { get; set; }

        public double MaxKlineHeight { get; set; }

        public int MaxMinKlines { get; set; }

        public ThinKlineParams(double klinesRatio, double maxKlineHeight, decimal dealProfit, int maxMinKlines) : base(0, 1, 0, 0)
        {
            KlinesRatio = klinesRatio;
            MaxKlineHeight = maxKlineHeight;
            DealProfit = (double)dealProfit;
        ...
        public override string ToString()
        {
            ...
            var conditions = $"kr\t{KlinesRatio}\tmkh\t{MaxKlineHeight}\tdp\t{DealProfit:0.####}\tmmk\t{MaxMinKlines:0.###}";
            return results + "\t" + conditions;
        }

        public override string GetParamsDesciption()
        {
            return $"kr\t{KlinesRatio}\tmkh\t{MaxKlineHeight}\tdp\t{DealProfit:0.####}\tmmk\t{MaxMinKlines:0.###}";
        }
    }


    internal class ThinKline : BaseStrategy
    {
        ...
        private long _lastStopMinTime = -1;

        private decimal _dealProfit = 1.001M;
        private double _twoKlinesRatio = 3;
        private double _maxKlineHeight = 0.01; // 0.005

        public ThinKline(string symbolName, AccountManager manager) : base(symbolName, manager)
        {
            BnnUtils.Log($"{GetName()}", false); //  {SymbolName} {_emaLength}
            _leftKlines = 7;
            _rightKlines = _leftKlines;
            _dealParams = new ThinKlineParams(_twoKlinesRatio, _maxKlineHeight, _dealProfit, _rightKlines);
            _isLimit = true;
        }

        public override string GetName() { return $"Thin Kline - {SymbolName}"; }
        #endregion
        ...
        private void CheckLong(List<Kline> klines)
        {
            if (_lastStopMinTime > 0) return;
            var lastKline = klines[^1];
            var allKlinesHigher = true;
            for (var i = klines.Count - 2; i > 0; i--)
            {
                allKlinesHigher = klines[i].OpenPrice > lastKline.OpenPrice;
                if (!allKlinesHigher) break;
                if (klines[i].OpenTime <= _lastMaximum.Time) break;
            }

            var klineIsThin = Math.Abs(lastKline.ClosePrice - lastKline.OpenPrice) / lastKline.OpenPrice < (decimal)_maxKlineHeight;

            var lastKlineIsLess = Math.Abs(lastKline.ClosePrice - lastKline.OpenPrice) / Math.Abs(klines[^2].ClosePrice - klines[^2].OpenPrice) < (decimal)_twoKlinesRatio;
            if (lastKlineIsLess && klineIsThin && allKlinesHigher && (_currentExtremum == 0)) EnterLong(lastKline);
        }


        private void CheckShort(Kline lastKline)
        {
            var isStopLoss = lastKline.LowPrice < _stopLossPrice;
            var isShort = lastKline.HighPrice > _shortPrice;
            if (isStopLoss && isShort)
            {
                isShort = lastKline.OpenPrice > lastKline.ClosePrice;
                isStopLoss = !isShort;
            }

            if (isStopLoss) _lastStopMinTime = _lastMinimum.Time;
            if (isStopLoss || isShort) EnterShort(lastKline, isStopLoss);
        }
        ...
        private void ProcessBacktestKline(List<Kline> klines)
        {
            ...
            if (_lastMinimum.Time > _lastStopMinTime) _lastStopMinTime = -1;
            if (_lastMaximum.Value >= _currentExtremum) _currentExtremum = 0;

            if ((decimal)lastKline.HighPrice > _lastMaximum.Value) _currentExtremum = (decimal)lastKline.HighPrice;

            if (AccountManager.LogLevel > 9) Log($"{lastKline}");
            if (AccountManager.LogLevel > 5) Log($"-{penultMinimum.Value} +{penultMaximum.Value} -{_lastMinimum.Value}{(_lastMaximum.Time < _lastMinimum.Time ? "*" : "")} +{_lastMaximum.Value}{(_lastMaximum.Time > _lastMinimum.Time ? "*" : "")} CE: {_currentExtremum}");

            if (!IsDealEntered && (_lastMinimum.Value < penultMinimum.Value) && (_lastMaximum.Value < penultMaximum.Value)) return;

            if (IsDealEntered) CheckShort(lastKline);
            else CheckLong(klines);
        }


        private void EnterLong(Kline kline)
        {
            _longPrice = (decimal)kline.ClosePrice;
            if (AccountManager.LogLevel > 3) Log($"enter long * {kline}; LP: {_longPrice}"); // ; rsi {_rsi:0.###}
            IsDealEntered = true;
            _shortPrice = _longPrice * _dealProfit;
            _stopLossPrice = _longPrice / _dealProfit;
        }


        private void EnterShort(Kline kline, bool isStop)
        {
            if (AccountManager.LogLevel > 3) Log($"{(isStop ? "stoploss :( " : "enter short :)")} {kline}"); // ; rsi {_rsi:0.###} *
            IsDealEntered = false;
            ...
            if (AccountManager.LogLevel > 3) Log($"{currPrice:0.###} / {_longPrice} * {balance:0.###} * {1 - Slippage}^2 = {newBalance:0.###}");
        ...
        {
            double[] klinesRatios = [0.1, 0.3, 0.5, 0.7, 3];
            double[] maxKlineHeights = [0.003, 0.005, 0.01, 0.03, 0.05];
            decimal[] dealProfits = [1.0005M, 1.001M, 1.003M, 1.005M, 1.01M];
            int[] maxMinKlines = [2, 3, 4, 5, 7];

            /* double[] klinesRatios = [0.5];
            double[] maxKlineHeights = [0.05];
            decimal[] dealProfits = [1.0005M, 1.001M, 1.01M];
            int[] maxMinKlines = [3];*/
            // decimal slValues !!!

            var deals = new List<BaseDealParams>();
            long paramsSetCount = klinesRatios.Length * maxKlineHeights.Length * dealProfits.Length * maxMinKlines.Length; //  * shortAngles.Length
            long counter = 0;
            foreach (var kr in klinesRatios)
            {
                _twoKlinesRatio = kr;
                foreach (var mkh in maxKlineHeights)
                {
                    _maxKlineHeight = mkh;
                    foreach (var dp in dealProfits)
                    {
                        _dealProfit = dp;
                        foreach (var mmk in maxMinKlines)
                        {
                            _leftKlines = mmk;
                            _rightKlines = mmk;
                            var tkp = new ThinKlineParams(_twoKlinesRatio, _maxKlineHeight, _dealProfit, _leftKlines);
                            _dealParams = tkp;
                            _order = null;
                            InitBacktest(null);
